...
CREATE POLICY IF NOT EXISTS "Everyone can view active products" ON public.products
...
CREATE POLICY IF NOT EXISTS "Everyone can view price history" ON public.price_history
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "System can insert price history" ON public.price_history
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO price_history (product_id, price, type)
  VALUES (NEW.id, NEW.price, 'listing');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION public.record_bid_price()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO price_history (product_id, price, type)
  VALUES (NEW.product_id, NEW.amount, 'bid');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
