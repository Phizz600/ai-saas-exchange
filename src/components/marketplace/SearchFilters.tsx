...
export const SearchFilters = ({
  ...
  const renderActiveFilters = () => {
    ...
    if (showAuctionsOnly) {
      filters.push(<Badge key="auctions" variant="secondary" className="gap-1 bg-amber-100 text-amber-800 hover:bg-amber-200">
          Auctions Only
          <X className="h-3 w-3 cursor-pointer" onClick={() => setShowAuctionsOnly(false)} />
        </Badge>);
    }
    if (showBuyNowOnly) {
      filters.push(<Badge key="buynow" variant="secondary" className="gap-1 bg-blue-100 text-blue-800 hover:bg-blue-200">
          Buy It Now Only
    ...
    if (sortBy !== 'relevant') {
      ...
      filters.push(<Badge key="sort" variant="secondary" className="gap-1 bg-purple-100 text-purple-800 hover:bg-purple-200">
          Sort: {sortLabel}
  ...
  return <div className="space-y-3 md:space-y-4 max-w-full">
      <div className="flex flex-col gap-3 md:gap-4 w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full">
          <Select value={sortBy} onValueChange={setSortBy}>
            ...
            <SelectContent>
              {sortOptions.map(option => <SelectItem key={option.value} value={option.value} className="flex items-center">
            ...
            <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
              <SheetTrigger asChild>
                <Button variant="outline" className="bg-gray-50 border-gray-200 shadow-sm hover:shadow-md flex-shrink-0 min-h-[40px]">
                  ...
                  Filter
                  {hasActiveFilters && <Badge variant="secondary" className="ml-2 bg-[#8B5CF6] text-white">
              ...
              <SheetContent className="w-full sm:max-w-md overflow-y-auto">
                ...
                <div className="py-6 space-y-6">
                  <FilterSection label="Industry" options={industries} value={industryFilter} onValueChange={setIndustryFilter} />
                  <FilterSection label="Stage" options={stages} value={stageFilter} onValueChange={setStageFilter} />
                  <FilterSection label="Price Range" options={priceRanges} value={priceFilter} onValueChange={setPriceFilter} />
                  <FilterSection label="Sort By" options={sortOptions} value={sortBy} onValueChange={setSortBy} />
                  <div className="flex flex-col space-y-2">
                    <div className="text-sm font-medium mb-1">Listing Type</div>
                    <div className="flex items-center space-x-2">
                      <Toggle pressed={showBuyNowOnly} onPressedChange={handleBuyNowToggle} variant="outline" className="flex-1 justify-start bg-white border-gray-200 hover:bg-blue-50 data-[state=on]:bg-blue-100 data-[state=on]:text-blue-800 min-h-[44px]">
                        <Tag className="h-4 w-4 mr-2" />
                        Buy It Now Only
                      </Toggle>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Toggle pressed={showAuctionsOnly} onPressedChange={handleAuctionsToggle} variant="outline" className="flex-1 justify-start bg-white border-gray-200 hover:bg-amber-50 data-[state=on]:bg-amber-100 data-[state=on]:text-amber-800 min-h-[44px]">
                        <Timer className="h-4 w-4 mr-2" />
                        Auctions Only
                      </Toggle>
                    </div>
                  </div>
                  {hasActiveFilters && <Button variant="destructive" className="w-full mt-4 min-h-[44px]" onClick={() => {
                  clearAllFilters();
                  setIsSheetOpen(false);
                }}>
                      Clear All Filters
      ...
      {/* Active Filters */}
      {hasActiveFilters && activeFilters.length > 0 && <motion.div className="flex flex-wrap gap-2 pt-1 -mx-3 sm:mx-0 px-3 sm:px-0 overflow-x-auto pb-2 sm:pb-0" initial={{
      opacity: 0,
      y: -10
    }} animate={{
      opacity: 1,
      ...
      duration: 0.3
          ...
          {activeFilters}
          {activeFilters.length > 1 && <Badge variant="outline" className="cursor-pointer bg-red-50 text-red-600 hover:bg-red-100 transition-colors flex-shrink-0" onClick={clearAllFilters}>
              Clear All <X className="h-3 w-3 ml-1" />
            </Badge>}
        </motion.div>}

      {isLoading && <div className="flex items-center text-sm text-gray-500">
