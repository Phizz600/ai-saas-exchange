...
export function IntegrationsField({
  form
}: IntegrationsFieldProps) {
  ...
  return <>
      <FormField control={form.control} name="integrations" render={() => <FormItem>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {INTEGRATIONS.map(integration => <FormField key={integration.id} control={form.control} name="integrations" render={({
          field
        }) => {
      ...
      {hasOther && <FormField control={form.control} name="integrations_other" render={({
      field
    }) => <FormItem>
