...
export function FinancialSection({
  ...
  const addCommonExpense = (expenseName: string, category: string) => {
    ...
    if (!exists) {
      const newExpense: ExpenseItem = {
        ...
        name: expenseName,
        amount: 0,
        // Default to 0, user can update later
        category
      };
      const updatedExpenses = [...monthlyExpenses, newExpense];
      form.setValue('monthlyExpenses', updatedExpenses);
  ...
  return <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-slate-50">Financials</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <FormField control={form.control} name="monthlyRevenue" render={({
        field
      }) => <FormItem>
              ...
              <FormControl>
                <Input placeholder="Enter monthly revenue in USD" value={field.value ? formatValue(field.value.toString()) : ''} onChange={e => {
        ...
        <FormField control={form.control} name="monthlyProfit" render={({
        field
      }) => <FormItem>
              <FormLabel className="flex items-center gap-2">
                Monthly Profit
              ...
              <FormControl>
                <Input placeholder="Enter monthly profit in USD" value={field.value ? formatValue(field.value.toString()) : ''} onChange={e => {
            const value = parseValue(e.target.value);
            if (value >= 0) {
              field.onChange(value);
            }
        ...
        <FormField control={form.control} name="customerAcquisitionCost" render={({
        field
      }) => <FormItem>
              <FormLabel className="flex items-center gap-2">
              ...
              <FormControl>
                <Input placeholder="Enter CAC in USD" value={field.value ? formatValue(field.value.toString()) : ''} onChange={e => {
            const value = parseValue(e.target.value);
            if (value >= 0) {
              field.onChange(value);
            }
        ...
        <FormField control={form.control} name="monetization" render={({
        field
      }) => <FormItem>
              <FormLabel className="flex items-center gap-2">
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent position="item-aligned" className="bg-white">
                  {MONETIZATION_OPTIONS.map(option => <SelectItem key={option} value={option}>
        ...
        {showMonetizationOther && <FormField control={form.control} name="monetizationOther" render={({
        field
      }) => <FormItem>
        ...
        <FormField control={form.control} name="grossProfitMargin" render={({
        field
      }) => <FormItem>
              <FormLabel className="flex items-center gap-2">
                Gross Profit Margin (%)
              ...
              <FormControl>
                <Input type="number" placeholder="Enter profit margin" value={field.value ?? ''} onChange={e => {
        ...
        <FormField control={form.control} name="monthlyChurnRate" render={({
        field
      }) => <FormItem>
              <FormLabel className="flex items-center gap-2">
                Monthly Churn Rate (%)
              ...
              <FormControl>
                <Input type="number" placeholder="Enter churn rate" value={field.value ?? ''} onChange={e => {
            const value = Number(e.target.value);
            field.onChange(isNaN(value) ? undefined : value);
          }} min="0" max="100" />
              </FormControl>
      ...
      <div className="mt-8 border-t pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
          ...
          <div className="md:col-span-3">
            <Input placeholder="Amount ($)" value={newExpenseAmount} onChange={e => setNewExpenseAmount(e.target.value)} />
          </div>
          <div className="md:col-span-3">
        ...
        {/* Expenses List */}
        {monthlyExpenses.length > 0 ? <div className="border rounded-md overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {monthlyExpenses.map((expense, index) => <tr key={expense.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-4 py-2 border-t">
                      <Input value={expense.name} onChange={e => {
                  ...
                  form.setValue('monthlyExpenses', updatedExpenses);
                    ...
                    <td className="px-4 py-2 border-t">
                      <Input value={expense.amount ? formatValue(expense.amount.toString()) : ''} onChange={e => {
                  const value = parseValue(e.target.value);
                  if (!isNaN(value)) {
                    const updatedExpenses = [...monthlyExpenses];
                    ...
                    <td className="px-4 py-2 border-t text-right">
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeExpense(expense.id)} className="h-8 w-8">
          ...
          </div> : <div className="text-center py-6 border rounded-md bg-gray-50">
