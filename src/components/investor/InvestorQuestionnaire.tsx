...
import { ChevronLeft, ChevronRight, Target, DollarSign, BarChart, CreditCard, TrendingUp, Building2, Bot, Users, Clock } from "lucide-react";
...
export const questions = [{
  ...
  options: [{
    ...
    value: "small",
    label: "Small ($50K - $200K)"
  }, {
    value: "medium",
    label: "Medium ($200K - $1M)"
  }, {
    value: "large",
    label: "Large ($1M+)"
  }],
...
export const InvestorQuestionnaire = ({
  ...
  if (hasCompletedQuestionnaire) {
    ...
    return <Card className={`p-6 text-center ${bgClass} ${className}`}>
  ...
  if (!showQuestionnaire) {
    ...
    return <Card className={`p-6 text-center ${bgClass} ${className}`}>
        <div className="flex flex-col items-center space-y-4">
          ...
          <Button onClick={() => setShowQuestionnaire(true)} className="bg-gradient-to-r from-[#D946EE] via-[#8B5CF6] to-[#0EA4E9] hover:opacity-90 transition-opacity">
            Start Questionnaire
          </Button>
        </div>
      </Card>;
  ...
  const bgClass = variant === "comingSoon" ? "bg-white/90 backdrop-blur-sm" : "";
  return <Card className={`p-6 max-w-2xl mx-auto ${bgClass} ${className}`}>
      <div className="space-y-6">
        ...
        <Form {...form}>
          ...
          field
            ...
            }} defaultValue={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {currentQ.options.map(option => <div key={option.value} className="relative">
                        <RadioGroupItem value={option.value} id={option.value} className="peer sr-only" />
                        <Label htmlFor={option.value} className="flex flex-col items-center justify-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-[#8B5CF6] peer-checked:bg-purple-50 transition-all duration-300">
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <Button variant="outline" onClick={handlePreviousQuestion} disabled={currentQuestion === 0} className="flex items-center gap-2">
        ...
        }} disabled={!form.getValues(`question_${currentQuestion}`)} className="bg-gradient-to-r from-[#D946EE] via-[#8B5CF6] to-[#0EA4E9] hover:opacity-90 transition-opacity flex items-center gap-2">
