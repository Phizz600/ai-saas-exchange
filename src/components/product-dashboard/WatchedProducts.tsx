...
export const WatchedProducts = () => {
  const {
    ...
    queryFn: async () => {
      ...
      const {
        ...
        `).in('id', profile.saved_products).order('created_at', {
        ascending: false
      }).limit(4);
      if (error) {
  ...
  if (!savedProducts?.length) {
    return <Card className="flex items-center justify-center p-8 bg-white/90">
        <p className="text-gray-500 text-lg">No listings saved at the moment</p>
      </Card>;
  }
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {savedProducts.map(item => <Link to={`/product/${item.product.id}`} key={item.id}>
          <Card className="p-4 hover:shadow-lg transition-shadow">
            <div className="aspect-video relative mb-3">
              {item.product.image_url ? <img src={item.product.image_url} alt={item.product.title} className="w-full h-full object-cover rounded-md" /> : <div className="w-full h-full bg-gray-100 rounded-md flex items-center justify-center">
