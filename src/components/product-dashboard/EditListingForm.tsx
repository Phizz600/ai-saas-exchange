...
export const EditListingForm = () => {
  ...
  const getCompletionScore = (product: any) => {
    ...
    const keyFields = ['title', 'description', 'price', 'category', 'stage', 'industry', 'monthly_revenue', 'monthly_traffic', 'active_users', 'tech_stack', 'team_size', 'competitors', 'demo_url'];
    const optionalFields = ['monthly_profit', 'gross_profit_margin', 'customer_acquisition_cost', 'monetization', 'business_model', 'product_age', 'image_url'];
  ...
  const getMissingFields = (product: any): string[] => {
    ...
    return Object.entries(importantFields).filter(([key, _]) => {
      const value = product[key];
      return !value || value === '' || Array.isArray(value) && value.length === 0 || typeof value === 'number' && value <= 0;
    }).map(([_, label]) => label);
  };
  ...
  return <div className="space-y-6">
      ...
      {selectedProduct ? <Card className="p-6">
          ...
          {getMissingFields(selectedProduct).length > 0 && <div className="mb-6 p-4 border border-amber-200 bg-amber-50 rounded-md">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ul className="list-disc pl-5 text-sm text-amber-700">
                    {getMissingFields(selectedProduct).map((field, index) => <li key={index}>{field}</li>)}
                  </ul>
                </div>
              </div>
            </div>}
            ...
            {fieldGroups.map(group => <TabsContent key={group.name} value={group.name.toLowerCase().replace(/\s/g, '-')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {group.fields.map(field => <div key={field.key} className="space-y-1">
                      ...
                      {field.type === 'image' && selectedProduct[field.key] ? <div className="relative h-36 w-36 rounded-md overflow-hidden border">
                          <img src={selectedProduct[field.key]} alt={selectedProduct.title} className="h-full w-full object-cover" />
                        </div> : field.type === 'array' && Array.isArray(selectedProduct[field.key]) ? <p className="font-medium">
