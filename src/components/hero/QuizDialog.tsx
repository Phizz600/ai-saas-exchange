...
export const QuizDialog = ({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        ...
        {!showResults && !showConfirmation && <div className="space-y-6">
            <div className="font-semibold text-lg">
              {quizQuestions[currentQuestion - 1].question}
            </div>
            <div className="space-y-3">
              {quizQuestions[currentQuestion - 1].options.map(option => <button key={option.value} onClick={() => handleOptionSelect(currentQuestion, option.value)} className={`w-full text-left p-4 rounded-lg border transition-all ${answers[currentQuestion] === option.value ? "border-[#6366f1] bg-[#f0f4ff]" : "border-gray-200 hover:border-[#6366f1] hover:bg-[#f5f7ff]"}`}>
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button onClick={handleNext} className="bg-[6366F1] bg-indigo-500 hover:bg-indigo-400">
                {currentQuestion === quizQuestions.length ? <>Calculate My Valuation</> : <>Next <ChevronRight className="ml-2 h-4 w-4" /></>}
              </Button>
            </div>
          </div>}

        {showResults && !showConfirmation && <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input type="text" placeholder="Your AI SaaS Company" value={formData.company} onChange={e => setFormData({
              ...formData,
              ...
              <label className="flex items-start space-x-2">
                <input type="checkbox" checked={formData.sellingInterest} onChange={e => setFormData({
              ...formData,
              sellingInterest: e.target.checked
            })} className="mt-1" />
        ...
        {showConfirmation && <div className="text-center space-y-4 py-4">
