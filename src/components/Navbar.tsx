            ...
            <Sheet>
              ...
              <SheetContent side="left" className="w-[300px] bg-[#EBEFF1]">
                ...
                <div className="flex flex-col gap-4 mt-8">
                  {navigationItems.map(item => <Link key={item.title} to={item.href} onClick={e => item.requiresAuth ? handleNavigationClick(e, item.href) : null} className="text-black hover:text-black/80 font-exo text-lg font-bold px-4 rounded-lg hover:bg-white/10 transition-colors py-0">
                      {item.title}
                    </Link>)}
                  {isAuthenticated && <Button onClick={handleSignOut} variant="secondary" className="mt-4 bg-gradient-to-r from-[#D946EE] via-[#8B5CF6] to-[#0EA4E9] text-white hover:opacity-90">
                      Sign Out
                    </Button>}
                </div>
              </SheetContent>
