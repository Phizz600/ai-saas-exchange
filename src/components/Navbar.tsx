            ...
            <Sheet>
              ...
              <SheetContent side="left" className="w-[300px] bg-[#EBEFF1] flex flex-col">
                ...
                <div className="flex flex-col gap-4 mt-8 flex-grow">
                  {navigationItems.map(item => <Link key={item.title} to={item.href} onClick={e => item.requiresAuth && !isAuthenticated ? handleNavigationClick(e, item.href) : null} className="text-black hover:text-black/80 font-exo text-lg px-4 rounded-lg hover:bg-white/10 transition-colors py-0">
                      {item.title}
                    </Link>)}
                </div>
                <div className="mt-auto mb-6">
                  <Button onClick={e => handleNavigationClick(e, '/list-product')} className="w-full bg-gradient-to-r from-[#D946EE] via-[#8B5CF6] to-[#0EA4E9] text-white hover:opacity-90" variant="secondary">Free AI SaaS Valuation</Button>
                </div>
              </SheetContent>
            </Sheet>
          </div>
