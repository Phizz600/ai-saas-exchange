      ...
      <div className="container mx-auto px-4 py-8">
        ...
        duration: 0.5
          ...
          <div className="flex flex-col items-center space-y-4">
            <Link to="/">
              <motion.img src="/lovable-uploads/f74b20e6-6798-4aeb-badd-2da6c2dce40b.png" alt="AI Exchange Logo" className="w-20 h-20 object-contain" animate={{
              y: [0, -10, 0]
            }} transition={{
              duration: 3,
              repeat: Infinity,
            ...
            {isLoading ? <div className="w-full p-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-md">
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 border-2 border-blue-300 border-t-transparent rounded-full animate-spin"></div>
                  <p className="text-white/80">Checking authentication status...</p>
                </div>
              </div> : isAuthenticated === false ? <div className="w-full p-4 bg-amber-900/20 backdrop-blur-md border border-amber-500/20 rounded-md text-amber-200">
      ...
      <Dialog open={helpDialogOpen} onOpenChange={setHelpDialogOpen}>
        <DialogContent className="sm:max-w-md glass backdrop-blur-lg bg-white/10 border border-white/20 text-white">
          ...
          <DialogFooter className="flex justify-between sm:justify-between">
            <Button variant="outline" onClick={() => setHelpDialogOpen(false)} className="bg-white/10 border-white/20 text-white hover:bg-white/20 transition-colors">
              Close
            </Button>
            <Button className="bg-gradient-to-r from-[#D946EE] via-[#8B5CF6] to-[#0EA4E9] hover:opacity-90 text-white" asChild>
              <a href="https://calendly.com/aiexchangeclub/listing-walkthrough" target="_blank" rel="noopener noreferrer">
                Book a Free Call
              </a>
            </Button>
          </DialogFooter>
